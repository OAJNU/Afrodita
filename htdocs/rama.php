...
class rama{
	...
	function rama($_id,$_titulo,$_hijos,$_padre,$_profundidad,$id_activo){
		...
		if($this->hijos>0){
			...
			if($this->id==NULL) {
				$strcns="SELECT id,titulo,hijos FROM ".lang_table()." WHERE padre IS NULL ORDER BY orden ASC";
			}
			else $strcns="SELECT id,titulo,hijos FROM ".lang_table()." WHERE padre=".$this->id." ORDER BY orden ASC";
	...
	function displayMapaSitio(){
		...
		if($this->titulo!="Nodo Padre"){
			...
			if($this->padre->padre==NULL) echo "<a href='".$FILE."?item_activo=".$this->id."&lang=".$GLOBALS['lang']."' class='subtitulo1'><span  style='text-transform:uppercase;'><strong>".$this->titulo."</strong></span></a></td></tr>";
			//si no es seccion principal
			else echo "<a href='".$FILE."?item_activo=".$this->id."&lang=".$GLOBALS['lang']."' class='texto_base'>".$this->titulo."</a></td></tr>";
		}
	...
	function getHTMLOutput($tipo_contenido){
		...
		$link="<a href='".$FILE."?item_activo=".$this->id."&lang=".$GLOBALS['lang']."' class='menu_item'>";

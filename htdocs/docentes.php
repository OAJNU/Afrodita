...
include ("rama_docentes.php"); //CLASE RAMA1
include ("docentes_menu.php");
...
function esContenido($item_activo){
	$strcns="SELECT * from contenidos_docentes WHERE id=".$item_activo;
...
function getIdPrimerHijo(){
	$strcns="SELECT id from contenidos_docentes WHERE padre=".$GLOBALS['objeto_activo']->id." ORDER BY orden LIMIT 0,1";
...
function selectItemPrincipal(){
    $strcnshtml="SELECT min(orden), id FROM contenidos_docentes WHERE padre IS NULL GROUP BY orden";
...
function mostrarContenido(){
	...
	/*  1. link
		...
		case 3: //TIPO PADRE VACIO
			header('Location: docentes.php?item_activo='.getIdPrimerHijo());
			break;
		case 4: //TIPO CVI
			return leerContenidoHtml($descripcion);
			break;
...
$ancho_menu=198;
$link=conectarDB();
$strcns="SELECT * FROM contenidos_docentes WHERE padre IS NULL";
...
<!-- fwtable fwsrc="menu.png" fwbase="menu.png" fwstyle="Dreamweaver" fwdocid = "1443153241" fwnested="1" -->
  ...
  <tr>
    <td width="759"><table width="759" border="0" align="left" cellpadding="0" cellspacing="0">
	  <tr>
		<td height="100%" valign="top"><? menu()?></td>
